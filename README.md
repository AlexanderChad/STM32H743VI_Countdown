...
Вывод оставшегося времени происходит на две линии по 5 матриц (HUB75, RGB 16x16, SM16126D), т.е. на экран 80x32 пикселя. На верхней линии по центру `количество дней`, на нижней - `чч:мм:сс`.  
...
Актуальное время получается от спутников с помощью любого GPS-модуля с выводом на UART сообщений стандарта NMEA (разъем `GPS`, Baud Rate: `115200`). Коррекция часового пояса задается настройками.  
